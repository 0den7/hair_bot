# HairBot — сервис онлайн-записи к мастеру
...
Веб-календарь будет доступен по адресу:
...
### 1. Создать файл `.env` в корне проекта (см. `.env.example`).
...
### 3. Применить миграции и заполнить базу начальными данными:
...
```text
https://hairbot.duckdns.org/docs
```
